...
<template>
    <div class="relative">
        <div class="fixed top-0 w-64 h-screen bg-white dark:bg-gray-800 z-20 transform duration-300 dark:text-gray-300" :class="{ '-translate-x-full': !show }">
            <Sidebar />
        </div>
        ...
        <div class="bg-gray-100 dark:bg-gray-900 h-screen overflow-hidden duration-300" :class="{ 'xl:pl-64': show }">
            <div class="flex items-center justify-between bg-white dark:bg-gray-800 rounded shadow m-4 p-4">
                <MenuIcon class="h-6 w-6 text-gray-600 dark:text-gray-300 cursor-pointer" @click="show = !show"></MenuIcon>
                <div class="flex items-center space-x-4">
                    <MoonIcon class="h-7 w-7 text-gray-600 cursor-pointer" @click="changeMode('dark')" v-if="theme === 'light'"></MoonIcon>
                    <SunIcon class="h-7 w-7 text-gray-300 cursor-pointer" @click="changeMode('light')" v-else></SunIcon>
                    <DropdownMenu />
                </div>
            </div>
